...
def dump_in_span(
    ...
    Args:
        ...
        fill_until_end: If True, ffills the last value until the end of the span if required.
    ...
    if isinstance(span[0], datetime):
        ...
        for name, series in vars_dict.items():
            ...
            # Fill until end if required
            if fill_until_end:
                if value.index[-1] < dt_end:
                    # last_value = series.iloc[-1]
                    value = series.reindex(pd.date_range(start=value.index[0], end=dt_end, freq=value.index.freq), method="ffill")
                    # span_vars_dict[name].iloc[-1] = last_value
                    
            span_vars_dict[name] = value
...
class EnvironmentVariables:
    ...
    def get_span(self) -> tuple[datetime, datetime]:
        """ Returns tuple of datetimes representing the range of the available data """
        index = list(asdict(self).values())[0].index
...
class RealDecVarsBoxBounds:
    ...
    def __post_init__(self) -> None:
        """ Validate that all bounds are tuples of (lower, upper) """
        for name, value in asdict(self).items():
            if not isinstance(value, tuple) or len(value) != 2:
                raise ValueError(f"Bounds for {name} must be a tuple of (lower, upper). Got {value}")
            if value[0] >= value[1]:
                raise ValueError(f"Lower bound must be less than upper bound for {name}. Got {value}")
...
class DecisionVariables:
    ...
    def from_dataframe(cls, df: pd.DataFrame, ) -> "DecisionVariables":
        ...
        for fld in fields(cls):
            if fld.name == "sfts_mode":
                value = df["sf_state"].fillna(0.0).astype(int)
            elif fld.name == "med_mode":
                value = (df["med_state"] > 0).fillna(0.0).astype(int)
            else:
                value = df[f"{fld.name}"]
            dec_vars_dict[fld.name] = value
            
        return cls(**dec_vars_dict)
    ...
    def replace_index_date(self, new_date: date) -> None:
        """ Method that given a datetime.date replaces the index of each object's attribute with it """
        
        for name, value in asdict(self).items():
            assert isinstance(value, pd.Series), f"This method requires all attributes to be timeseries, {name}: {type(value)}"
            
            value.index = value.index.map(lambda t: pd.Timestamp.combine(new_date, t.timetz()))
            setattr(self, name, value)
        # return DecisionVariables(**self.to_dict()) # Return the modified copy

    def get_dates(self, ) -> list[date]:
        """ Returns list of dates in object """
        assert isinstance(self.qsf, pd.Series) and isinstance(self.qsf.index, pd.DatetimeIndex), \
            "This method requires attributes to be of type pd.Series with a datetime index"
        return np.unique(self.qsf.index.date)
    
    def enforce_bounds(self, bounds: RealDecVarsBoxBounds) -> None:
        """ Enforce bounds on the real decision variables """
        
        from solarmed_optimization.utils import get_first_type_hint
        
        for name, value in asdict(self).items():
            if value is None:
                continue
            
            if get_first_type_hint(DecisionVariables, name) is int:
                # If the value is an integer, we can skip clipping
                continue
            ...
            if isinstance(value, pd.Series):
                # If value is a pd.Series, clip it
                value_clipped = value.clip(lower=bound[0], upper=bound[1])
            elif isinstance(value, np.ndarray):
            # If value is a single float or np.ndarray, clip it
                value_clipped = np.clip(value, bound[0], bound[1])
            elif isinstance(value, (list, tuple)):
                # If it's a single float, just use max and min
                value_clipped = max(min(value, bound[1]), bound[0])
            else:
                raise TypeError(f"Unsupported type for clipping: {type(value)}. Expected pd.Series, np.ndarray, list, or tuple.")
                
            setattr(self, name, value_clipped)
...
class IntegerDecisionVariables:
    ...
    def resample(self, target_freq_sec: int, *args, **kwargs) -> "IntegerDecisionVariables":
        ...
        for name, value in asdict(self).items():
            ...
            value = value.resample(f"{target_freq_sec}s", *args, **kwargs)
            # if  target_freq > current_freq: # Downsample
            # 	value = value.first()
            # else: # Upsample
            value = value.ffill()
    ...
    def get_dates(self, ) -> list[date]:
        """ Returns list of dates in object """
        assert isinstance(self.sfts_mode, pd.Series) and isinstance(self.sfts_mode.index, pd.DatetimeIndex), \
            "This method requires attributes to be of type pd.Series with a datetime index"
        return np.unique(self.sfts_mode.index.date)
    
    def update_index_date(self, date_delta: timedelta) -> None:
        """ Method that adds the provided datetime.timedelta to the index of each attribute """
        
        for name, value in asdict(self).items():
            assert isinstance(value, pd.Series), f"This method requires all attributes to be timeseries, {name}: {type(value)}"
            
            value.index += date_delta
            setattr(self, name, value)
