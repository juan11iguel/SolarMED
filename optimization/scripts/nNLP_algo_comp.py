...
max_n_obj_fun_evals: int = 20
pop_sizes: list[int] = [10, 20]#, 50, 100]
...
def build_and_evaluate_archipielago() -> None:
    ...
    for pop_idx, pop_size in enumerate(pop_sizes):
        ...
        for algo_id, algo_params in algorithms_to_eval.items():
            ...
            results_dict[f"{algo_id}_pop_{pop_size}"] = {"x0": pop0.champion_x,
    ...
    # 8. Save results
    with open(base_output_path / f"algo_comp_eval_at_{datetime.now():%Y%m%dT%H%M}_for_{date_str}_data.json", "w") as f:
        json.dump(results_dict, f, indent=4, cls=CustomEncoder)
    
    
if __name__ == "__main__":
    build_and_evaluate_archipielago()