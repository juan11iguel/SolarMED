...
class SolarMED(BaseModel):
    ...
    lims_Tsf_out: rangeType = Field((65, 110), title="Tsf,out setpoint limits", json_schema_extra={"units": "C"},
    ...
    # Thermal storage
    mts_src_sp: float = Field(None, title="mts,src*", json_schema_extra={"units": "m3/h"}, description="Decision variable. Thermal storage recharge flow rate (m³/h)")

    mts_src: float = Field(None, title="mts,src", json_schema_extra={"units": "m3/h"}, description="Output. Thermal storage recharge flow rate (m³/h)")
    mts_dis: float = Field(None, title="mts,dis", json_schema_extra={"units": "m3/h"}, description="Output. Thermal storage discharge flow rate (m³/h)")
    ...
    Pth_ts_in: float = Field(None, title="Pth,ts,in", json_schema_extra={"units": "kWth"}, description="Output. Thermal storage inlet power (kWth)")
    Pth_ts_out: float = Field(None, title="Pth,ts,out", json_schema_extra={"units": "kWth"}, description="Output. Thermal storage outlet power (kWth)")
    Jts_e: float = Field(None, title="Jts,e", json_schema_extra={"units": "kWe"}, description="Output. Thermal storage electrical power consumption (kWe)")
    ...
    msf: float = Field(None, title="msf", json_schema_extra={"units": "m3/h"}, description="Output. Solar field flow rate (m³/h)")
    SEC_sf: float = Field(None, title="SEC_sf", json_schema_extra={"units": "kWhe/kWth"}, description="Output. Solar field conversion efficiency (kWhe/kWth)")
    Jsf_e: float = Field(None, title="Jsf,e", json_schema_extra={"units": "kW"}, description="Output. Solar field electrical power consumption (kWe)")
    Pth_sf: float = Field(None, title="Pth_sf", json_schema_extra={"units": "kWth"}, description="Output. Solar field thermal power generated (kWth)")

    # MED
    mmed_s_sp: float = Field(None, title="mmed,s*", json_schema_extra={"units": "m3/h"}, description="Decision variable. MED hot water flow rate (m³/h)")
    mmed_f_sp: float = Field(None, title="mmed,f*", json_schema_extra={"units": "m3/h"}, description="Decision variable. MED feedwater flow rate (m³/h)")
    ...
    mmed_s: float = Field(None, title="mmed,s", json_schema_extra={"units": "m3/h"}, description="Output. MED hot water flow rate (m³/h)")
    mmed_f: float = Field(None, title="mmed,f", json_schema_extra={"units": "m3/h"}, description="Output. MED feedwater flow rate (m³/h)")
    ...
    mmed_c: float = Field(None, title="mmed,c", json_schema_extra={"units": "m3/h"}, description="Output. MED condenser flow rate (m³/h)")
    ...
    mmed_d: float = Field(None, title="mmed,d", json_schema_extra={"units": "m3/h"}, description="Output. MED distillate flow rate (m³/h)")
    mmed_b: float = Field(None, title="mmed,b", json_schema_extra={"units": "m3/h"}, description="Output. MED brine flow rate (m³/h)")
    Jmed_e: float = Field(None, title="Jmed,e", json_schema_extra={"units": "kWe"}, description="Output. MED electrical power consumption (kW)")
    Jmed_th: float = Field(None, title="Jmed,th", json_schema_extra={"units": "kWth"}, description="Output. MED thermal power consumption ~= Pth_ts_out (kW)")
    STEC_med: float = Field(None, title="STEC_med", json_schema_extra={"units": "kWhe/m3"}, description="Output. MED specific thermal energy consumption (kWhe/m³)")
    SEEC_med: float = Field(None, title="SEEC_med", json_schema_extra={"units": "kWhth/m3"}, description="Output. MED specific electrical energy consumption (kWhth/m³)")
    ...
    mhx_p: float = Field(None, title="mhx,p", json_schema_extra={"units": "m3/h"}, description="Output. Heat exchanger primary circuit (hot side) flow rate == msf (m³/h)")
    mhx_s: float = Field(None, title="mhx,s", json_schema_extra={"units": "m3/h"}, description="Output. Heat exchanger secondary circuit (cold side) flow rate == mts_src (m³/h)")
    Pth_hx_p: float = Field(None, title="Pth,hx,p", json_schema_extra={"units": "kWth"}, description="Output. Heat exchanger primary circuit (hot side) power == Pth_sf (kWth)")
    Pth_hx_s: float = Field(None, title="Pth,hx,s", json_schema_extra={"units": "kWth"}, description="Output. Heat exchanger secondary circuit (cold side) power == Pth_ts_in (kWth)")
    eta_hx: float = Field(None, title="𝜂hx", json_schema_extra={"units": "-"}, description="Output. Heat exchanger efficiency (-)")
    ...
    m3wv_src: float = Field(None, title="m3wv,src", json_schema_extra={"units": "m3/h"}, description="Output. Three-way valve source flow rate == mts,dis (m³/h)")
    m3wv_dis: float = Field(None, title="m3wv,dis", json_schema_extra={"units": "m3/h"}, description="Output. Three-way valve discharge flow rate == mmed,s (m³/h)")
    ...
    # Others
    MED_model: Any = Field(None, repr=False, exclude=True, description="MATLAB MED model instance")
    ...
    def solve_solar_field_inverse(self, Tsf_out):
        msf = solar_field_model_inverse(Tin=self.Tsf_in, Tout=Tsf_out, I=self.I,
                                        Tamb=self.Tamb, beta=self.beta_sf, H=self.H_sf)
    ...
    def solve_coupled_subproblem(self) -> tuple[float, float, float, np.ndarray, np.ndarray, float]:
        ...
        if cnt == 10:
            logger.debug(f"Not converged in {cnt} iterations, deltaTsf_out = {deltaTsf_out:.3f}")
        else:
    ...
    def step(self,
        ...
        if self.sf_active:
            ...
            self.Jsf_e = self.sf_actuators[0].calculate_power_consumption(self.msf)
            self.SEC_sf = self.Jsf_e / self.Pth_sf if self.Pth_sf > 0 else np.nan # kWhe/kWth
        else:
            self.Pth_sf = 0
            self.Jsf_e = 0
            self.SEC_sf = np.nan

        if self.ts_recharging:
            ...
            self.Jts_e = self.ts_actuators[0].calculate_power_consumption(self.mts_src)
        else:
            self.Pth_ts_in = 0
            self.Jts_e = 0
        ...
        self.eta_hx = self.Pth_hx_s / self.Pth_hx_p if self.Pth_hx_p > 0 else np.nan
