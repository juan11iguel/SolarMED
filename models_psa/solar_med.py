...
class SolarMED(BaseModel):
    ...
    beta_sf: float = Field(1.2416e-2, title="βsf", json_schema_extra={"units": "m"}, repr=False,
                           description="Solar field. Gain coefficient", gt=0, le=1)
    H_sf: float = Field(3.4084, title="Hsf", json_schema_extra={"units": "W/m2"}, repr=False,
                        description="Solar field. Losses to the environment", ge=0, le=20)
    gamma_sf: float = Field(0.0498, title="γsf", json_schema_extra={"units": "-"}, repr=False,
    ...
    Pth_ts_in: float = Field(None, title="Pth,ts,in", json_schema_extra={"units": "kWth"},
                             description="Output. Thermal storage inlet power (kWth)")
    Pth_ts_out: float = Field(None, title="Pth,ts,out", json_schema_extra={"units": "kWth"},
                              description="Output. Thermal storage outlet power (kWth)")
    Jts_e: float = Field(None, title="Jts,e", json_schema_extra={"units": "kWe"},
    ...
    Tsf_in: conHotTemperatureType = Field(..., title="Tsf,in", json_schema_extra={"units": "C"},
    ...
    msf: float = Field(None, title="msf", json_schema_extra={"units": "m3/h"},
                       description="Output. Solar field flow rate (m³/h)")
    ...
    Jsf_e: float = Field(None, title="Jsf,e", json_schema_extra={"units": "kW"},
                         description="Output. Solar field electrical power consumption (kWe)")
    Pth_sf: float = Field(None, title="Pth_sf", json_schema_extra={"units": "kWth"},
    ...
    Tmed_s_in_sp: float = Field(None, title="Tmed,s,in*", json_schema_extra={"units": "C"},
                                description="Decision variable. MED hot water inlet temperature (ºC)", ge=0, le=89)
    Tmed_c_out_sp: float = Field(None, title="Tmed,c,out*", json_schema_extra={"units": "C"},
                                 description="Decision variable. MED condenser outlet temperature (ºC)", ge=0)
    ...
    Tmed_s_in: float = Field(None, title="Tmed,s,in", json_schema_extra={"units": "C"},
                             description="Output. MED hot water inlet temperature (ºC)", ge=0, le=89)
    ...
    Jmed_e: float = Field(None, title="Jmed,e", json_schema_extra={"units": "kWe"},
                          description="Output. MED electrical power consumption (kW)")
    Jmed_th: float = Field(None, title="Jmed,th", json_schema_extra={"units": "kWth"},
    ...
    Pth_hx_p: float = Field(None, title="Pth,hx,p", json_schema_extra={"units": "kWth"},
                            description="Output. Heat exchanger primary circuit (hot side) power == Pth_sf (kWth)")
    Pth_hx_s: float = Field(None, title="Pth,hx,s", json_schema_extra={"units": "kWth"},
                            description="Output. Heat exchanger secondary circuit (cold side) power == Pth_ts_in (kWth)")
    eta_hx: float = Field(None, title="𝜂hx", json_schema_extra={"units": "-"},
                          description="Output. Heat exchanger efficiency (-)")
    ...
    msf_prior: np.ndarray[PositiveFloat] = Field(None, repr=False, exclude=False,
    ...
    def solve_thermal_storage(self, Tts_h_in: float, ) -> tuple[
        np.ndarray[conHotTemperatureType], np.ndarray[conHotTemperatureType]]:
    ...
    def solve_solar_field(self, Tsf_in: float, msf: float):
        ...
        """
        solar_field_model(
          Tin=ds['Tsf_in'], q=ds['qsf'], I=ds['I'], Tamb=ds['Tamb'],  Tout_ant=Tsf_out_mod[i-1],
        ...
        Tsf_out = solar_field_model(
            Tin=Tsf_in, Tout_ant=self.Tsf_out_ant, q=msf, q_ant=self.msf_prior, I=self.I, Tamb=self.Tamb,
            # Model tuned parameters
            H=self.H_sf, beta=self.beta_sf, gamma=self.gamma_sf,
            # Model fixed parameters
            Acs=self.Acs_sf, nt=self.nt_sf, np=self.np_sf, ns=self.ns_sf, Lt=self.Lt_sf,
    ...
    def step(self,
        ...
        if self.ts_recharging:
            ...
            self.Pth_ts_in = self.mts_src * w_props_ts_in.rho * (
                        self.Tts_c[-1] - self.Tts_h_in) * w_props_ts_in.cp / 3600  # kWth
        ...
        if self.med_active:
            ...
            self.Pth_ts_out = self.mts_dis * w_props_ts_out.rho * (
    ...
    def to_dataframe(self, df=None) -> pd.DataFrame:
        """
        Take all fields that are numeric and return them as a pandas DataFrame
        Additionally, thermal storage temperatures are expanded following the nomenclature
        ...
        if self.export_fields is None:
            ...
            self.export_fields = [field for field in self.__fields__.keys() if
                                  isinstance(getattr(self, field), (int, float))]

        # Create a dataframe from the dictionary
        data = pd.DataFrame(self.model_dump(include=self.export_fields), index=[0])
